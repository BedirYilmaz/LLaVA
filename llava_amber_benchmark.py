...
try:
    ...
    subprocess.run([sys.executable, "-m", "pip", "install", "transformers", "torch", "torchvision", "Pillow", "tqdm", "accelerate", "sentencepiece", "protobuf"])
    exit(1)


def load_llava_model(model_path="liuhaotian/llava-v1.6-mistral-7b", model_base=None, device='cuda', load_8bit=False, load_4bit=False):
...
def run_amber_benchmark(model_path, query_file, image_dir, output_file, max_samples=None, start_from=0):
    ...
    tokenizer, model, image_processor, context_len = load_llava_model(model_path)
...
def main():
    ...
    parser.add_argument("--output_file", type=str, default="llava_amber_responses.json",

#!/usr/bin/env python3
"""
ORPO Training Script for LLaVA v1.6 7B on RLAIF-V Dataset (TEST VERSION)
This version uses a small subset of data for quick testing
"""
...
def format_dataset_for_orpo(dataset, tokenizer, image_processor):
    ...
    def format_example(example):
        ...
        if hasattr(image_processor, 'size') and 'longest_edge' in image_processor.size:
            max_size = min(image_processor.size["longest_edge"], 224)  # Even smaller for testing
        else:
            max_size = 224
    ...
    formatted_dataset = dataset.map(
        ...
        num_proc=2,  # Reduced for testing
...
def main():
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        ...
        model_max_length=1024,  # Reduced for testing
        padding_side="right",
        use_fast=False,
    )
    ...
    # Configure LoRA (smaller for testing)
    peft_config = LoraConfig(
        r=64,  # Reduced from 128
        lora_alpha=128,  # Reduced from 256
        target_modules=["q_proj", "v_proj", "k_proj", "o_proj"],  # Reduced set
        lora_dropout=0.05,
        bias="none",
        task_type="CAUSAL_LM",
    )
    ...
    dataset = load_dataset(
        ...
        split="train[:100]",  # Only 100 samples for testing
    ...
    training_args = ORPOConfig(
        output_dir="./checkpoints/llava-v1.6-7b-orpo-test",
        num_train_epochs=1,
        ...
        dataloader_num_workers=2,  # Reduced
        remove_unused_columns=False,
        max_length=1024,  # Reduced
        max_prompt_length=512,  # Reduced
        beta=0.1,  # ORPO beta parameter
        dataset_num_proc=2,  # Reduced
        max_steps=50,  # Limit max steps for testing
    )
    ...
    # Start training
    logger.info("Starting ORPO training (TEST MODE)...")
    trainer.train()
    
    # Save the final model
    trainer.save_model()
    logger.info(f"Test training completed! Model saved to {training_args.output_dir}")
